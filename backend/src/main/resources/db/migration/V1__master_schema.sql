...
CREATE TABLE ATTRIBUTE(attribute_id uuid PRIMARY KEY, attribute_code varchar(50) UNIQUE NOT NULL, attribute_name varchar(100) NOT NULL, data_type varchar(30) NOT NULL, is_required bool DEFAULT FALSE, created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamp with time zone, CONSTRAINT chk_option_value_not_blank CHECK(length(trim(attribute_name)) > 0), CONSTRAINT chk_attribute_data_type CHECK (data_type IN ('STRING', 'INTEGER', 'DECIMAL', 'BOOLEAN', 'DATE', 'DATETIME', 'ENUM', 'JSON')));
CREATE TABLE ATTRIBUTE_OPTION(option_id uuid PRIMARY KEY, attribute_id uuid NOT NULL, option_code varchar(50) NOT NULL, display_name varchar(100) NOT NULL, display_order int4 NOT NULL DEFAULT 1, is_active bool DEFAULT TRUE, created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamp with time zone, CONSTRAINT unq_attribute_option UNIQUE (attribute_id, option_code), CONSTRAINT unq_attribute_display_name UNIQUE(attribute_id, display_name), CONSTRAINT chk_display_name CHECK(length(trim(display_name)) > 0), CONSTRAINT chk_display_order CHECK (display_order >= 0));
CREATE TABLE SKU_ATTRIBUTE(sku_attribute_id uuid PRIMARY KEY, sku_id uuid NOT NULL, attribute_id uuid NOT NULL, attribute_value varchar(255) NOT NULL, CONSTRAINT fk_sku_attribute_sku FOREIGN KEY(sku_id) REFERENCES SKU(sku_id), CONSTRAINT fk_sku_attribute FOREIGN KEY(attribute_id) REFERENCES ATTRIBUTE(attribute_id), CONSTRAINT unq_sku_attribute UNIQUE(sku_id, attribute_id));

CREATE TABLE ABC_CLASS (abc_code varchar(10) PRIMARY KEY NOT NULL, display_name varchar(50) NOT NULL, description text, min_percentage numeric(5,2), max_percentage numeric(5,2), priority smallint NOT NULL, color_code varchar(20), cycle_count_frequency_days int4 NOT NULL, target_service_level numeric(5,2), max_pick_distance int4, is_active bool NOT NULL DEFAULT TRUE, created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP, version int4 NOT NULL DEFAULT 0, CONSTRAINT chk_abc_priority CHECK(priority > 0), CONSTRAINT chk_abc_percentage
	CHECK (
