...
CREATE TABLE LPN(lpn_id uuid NOT NULL, location_id uuid, lpn_code varchar(50) NOT NULL UNIQUE, lpn_type varchar(30) NOT NULL, lpn_status varchar(50) NOT NULL, created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, version int4 DEFAULT 0 NOT NULL, PRIMARY KEY (lpn_id), CONSTRAINT chk_lpn_status CHECK (lpn_status IN ('AVAILABLE', 'ALLOCATED', 'STAGED', 'IN_TRANSIT')), CONSTRAINT chk_lpn_type CHECK (lpn_type IN ('PALLET', 'BOX', 'TOTE')));

CREATE TABLE LPN_DETAIL(lpn_detail_id uuid NOT NULL, lpn_id uuid NOT NULL, lot_id uuid NOT NULL, sku_id uuid NOT NULL, quantity numeric(18, 4) NOT NULL, created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, version int4 DEFAULT 0 NOT NULL, PRIMARY KEY (lpn_detail_id), CONSTRAINT chk_lpn_detail_quantity CHECK (quantity > 0));

CREATE TABLE INVENTORY_ADJUSTMENT(adjustment_id uuid NOT NULL, warehouse_id uuid NOT NULL, reference_id uuid, adjustment_code varchar(50) NOT NULL, adjustment_type varchar(50) NOT NULL, adjustment_status varchar(50) NOT NULL, adjustment_time timestamp with time zone DEFAULT CURRENT_TIMESTAMP, reason_code varchar(50) NOT NULL, description text, reference_type varchar(30) NOT NULL, requested_by varchar(50) NOT NULL, approved_by varchar(50), approved_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP, created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, created_by varchar(50), updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_by varchar(50), version int4 DEFAULT 0 NOT NULL, PRIMARY KEY (adjustment_id), CONSTRAINT chk_adjustment_approved CHECK ((approved_by IS NULL AND approved_at IS NULL) OR (approved_by IS NOT NULL AND approved_at IS NOT NULL)),
...
COMMENT ON COLUMN INVENTORY_ADJUSTMENT.reason_code IS 'for generating reports purposes';
...
CREATE TABLE INVENTORY_ADJUSTMENT_DETAIL(adjustment_detail_id uuid NOT NULL, adjustment_id uuid NOT NULL, location_id uuid NOT NULL, sku_id uuid NOT NULL, lot_id uuid NOT NULL, lpn_id uuid NOT NULL, quantity_before numeric(12, 4) DEFAULT 0 NOT NULL, quantity_after numeric(12, 4) DEFAULT 0 NOT NULL, adjusted_quantity numeric(12, 4) DEFAULT 0 NOT NULL, created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, version int4 DEFAULT 0 NOT NULL, PRIMARY KEY (adjustment_detail_id), CONSTRAINT chk_adjustment_detail_before_after CHECK (quantity_before >= 0 AND quantity_after >= 0), CONSTRAINT chk_adjusted_quantity CHECK (adjusted_quantity <> 0));
COMMENT ON COLUMN INVENTORY_ADJUSTMENT_DETAIL.adjusted_quantity IS 'adjusted_quantity = quantity_before - quantity_after';

CREATE TABLE CYCLE_COUNT(cycle_count_id uuid NOT NULL, warehouse_id uuid NOT NULL, cycle_count_code varchar(50) NOT NULL UNIQUE, count_type varchar(30) NOT NULL, count_status varchar(30) NOT NULL, scheduled_date date NOT NULL, started_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, completed_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, assigned_to varchar(100) NOT NULL, approved_by varchar(100) NOT NULL, approved_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, notes text, created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, created_by varchar(100) NOT NULL, updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP, updated_by varchar(100), version int4 DEFAULT 0 NOT NULL, PRIMARY KEY (cycle_count_id), CONSTRAINT chk_cycle_count_count_type CHECK (count_type IN ('ABC_BASED', 'LOCATION_BASED', 'SKU_BASED', 'RANDOM', 'DISCREPANCY_TRIGGERED')), CONSTRAINT chk_cycle_count_count_status CHECK (count_status IN ('PLANNED', 'IN_PROGRESS', 'PENDING_APPROVAL', 'APPROVED', 'CANCELLED', 'COMPLETED')));
...
CREATE TABLE CYCLE_COUNT_DETAIL (detail_id uuid NOT NULL, cycle_count_id uuid NOT NULL, discrepancy_reason_id uuid, location_id uuid NOT NULL, lot_id uuid NOT NULL, lpn_id uuid NOT NULL, sku_id uuid NOT NULL, system_quantity numeric(18, 4) DEFAULT 0 NOT NULL, system_quantity_snapshot timestamp with time zone NOT NULL, counted_quantity numeric(18, 4) DEFAULT 0, recounted_quantity numeric(18, 4) DEFAULT 0, variance_quantity numeric(18, 4) DEFAULT 0, detail_status varchar(30) NOT NULL, counted_by varchar(100), counted_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (detail_id), CONSTRAINT chk_cycle_count_detail_status CHECK (detail_status IN ('PENDING', 'COUNTED', 'RECOUNT_REQUESTED', 'MATCHED', 'MISMATCHED', 'APPROVED')));
...
COMMENT ON COLUMN CYCLE_COUNT_DETAIL.system_quantity_snapshot IS 'System quantity snapshot time';
...
CREATE TABLE INVENTORY_RESERVATION(reservation_id uuid NOT NULL, warehouse_id uuid NOT NULL, location_id uuid, sku_id uuid NOT NULL, lot_id uuid NOT NULL, lpn_id uuid NOT NULL, reference_doc_id uuid NOT NULL, sales_order_line_id uuid, reservation_code varchar(50) NOT NULL, reservation_type varchar(50) NOT NULL, reservation_status varchar(50) NOT NULL, reserved_quantity numeric(12, 4) DEFAULT 0 NOT NULL, allocated_quantity numeric(12, 4) DEFAULT 0 NOT NULL, picked_quantity numeric(12, 4) DEFAULT 0 NOT NULL, remaining_quantity numeric(12, 4) DEFAULT 0 NOT NULL, reference_doc_type varchar(30) NOT NULL, reserved_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, expires_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP, released_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP, fulfilled_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP, created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, created_by varchar(50) NOT NULL, updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_by varchar(50), version int4 DEFAULT 0 NOT NULL, PRIMARY KEY (reservation_id),
CONSTRAINT chk_reserved_quantity CHECK (reserved_quantity > 0 AND allocated_quantity >= 0 AND picked_quantity >= 0 AND remaining_quantity > 0),
CONSTRAINT chk_reservation_reference_type CHECK (reference_doc_type IN ('SO', 'TO')),
CONSTRAINT chk_reservation_type CHECK((reservation_type IN ('CART_RESERVATION', 'PICKING_ALLOCATION', 'REPLENISHMENT_BLOCK', 'VIRTUAL_PROMOTION', 'INTERNAL_QC_HOLD')) AND  (CASE WHEN reservation_type = 'CART_RESERVATION' THEN(sales_order_line_id IS NOT NULL AND expires_at IS NOT NULL AND location_id IS NULL) WHEN reservation_type = 'PICKING_ALLOCATION' THEN (sales_order_line_id IS NOT NULL AND expires_at IS NULL AND location_id IS NOT NULL) WHEN reservation_type IN ('REPLENISHMENT_BLOCK', 'INTERNAL_QC_HOLD') THEN (sales_order_line_id IS NULL AND location_id IS NOT NULL) ELSE TRUE END)),
...
CREATE TABLE INVENTORY_SNAPSHOT(snapshot_id uuid NOT NULL, sku_id uuid NOT NULL, lot_id uuid, lpn_id uuid, warehouse_id uuid NOT NULL, location_id uuid NOT NULL, snapshot_date date NOT NULL, snapshot_timestamp timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, snapshot_type varchar(30) NOT NULL, quantity_on_hand numeric(18, 4) DEFAULT 0 NOT NULL, quantity_allocated numeric(18, 4) DEFAULT 0 NOT NULL, quantity_available numeric(18, 4) DEFAULT 0 NOT NULL, quantity_hold numeric(12, 4) DEFAULT 0 NOT NULL, quantity_damaged numeric(12, 4) DEFAULT 0 NOT NULL, created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, version int4 DEFAULT 0 NOT NULL, PRIMARY KEY (snapshot_id),
CONSTRAINT chk_inventory_snapshot_quantity CHECK ((quantity_on_hand >= 0) AND (quantity_allocated >= 0) AND (quantity_available >= 0) AND (quantity_hold >= 0) AND (quantity_damaged >= 0)),
...
COMMENT ON COLUMN INVENTORY_SNAPSHOT.quantity_allocated IS 'The total on-hand quantity allocated to outbound demands.';
COMMENT ON COLUMN INVENTORY_SNAPSHOT.quantity_available IS 'quantity_available = quantity_on_hand - quantity_allocated - quantity_hold - quantity_damaged';
COMMENT ON COLUMN INVENTORY_SNAPSHOT.quantity_hold IS 'The quantity currently placed on quality hold or are currently under segregation/quarantine.';

CREATE TABLE INVENTORY_TRANSACTION(transaction_id uuid NOT NULL, sku_id uuid NOT NULL, lot_id uuid, lpn_id uuid, from_location_id uuid, to_location_id uuid, reference_doc_id uuid, operator_id uuid, transaction_code varchar(50) NOT NULL, transaction_type varchar(50) NOT NULL, transaction_subtype varchar(50) NOT NULL, quantity_change numeric(12, 4) NOT NULL, reference_doc_type varchar(30), reason_code varchar(50), transaction_time timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL, created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, created_by varchar(50) NOT NULL, updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_by varchar(50) NOT NULL, remarks text, version int4 DEFAULT 0 NOT NULL, PRIMARY KEY (transaction_id),
...
CONSTRAINT chk_transaction_type CHECK (transaction_type IN ('INBOUND_RECEIPT', 'OUTBOUND', 'INTERNAL_TRANSFER', 'ADJUSTMENT', 'COUNT', 'RETURN', 'QC', 'REPLENISHMENT', 'HOLD', 'RELEASE')),
...
COMMENT ON COLUMN INVENTORY_TRANSACTION.reason_code IS 'For processing adjustment orders purposes';
...
CREATE TABLE INVENTORY_TRANSFER(transfer_id uuid NOT NULL, from_warehouse_id uuid NOT NULL, to_warehouse_id uuid NOT NULL, transfer_code varchar(50) NOT NULL UNIQUE, transfer_type varchar(50) NOT NULL, priority int4 DEFAULT 1 NOT NULL, transfer_status varchar(50) NOT NULL, scheduled_date date NOT NULL, started_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, completed_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, requested_by varchar(100) NOT NULL, assigned_to varchar(100), approved_by varchar(100), approved_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP, notes text, created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, created_by varchar(100) NOT NULL, updated_by varchar(100), version int4 DEFAULT 0 NOT NULL, PRIMARY KEY (transfer_id),
...
CONSTRAINT chk_replenish_source_target_zone CHECK (source_zone_id <> target_zone_id),
...
ALTER TABLE INVENTORY_BALANCE ADD CONSTRAINT "fk_balance_warehouse_id" FOREIGN KEY (warehouse_id) REFERENCES WAREHOUSE(warehouse_id);
...
ALTER TABLE INVENTORY_BALANCE ADD CONSTRAINT "uk_inventory_balance" UNIQUE NULLS NOT DISTINCT(warehouse_id, location_id, sku_id, lot_id, lpn_id, inventory_status);
...
ALTER TABLE LPN_DETAIL ADD CONSTRAINT "uk_lpn_detail_lpn_sku_lot" UNIQUE(lpn_id, sku_id, lot_id);
...
ALTER TABLE INVENTORY_ADJUSTMENT_DETAIL ADD CONSTRAINT "fk_adjustment_detail_sku_id" FOREIGN KEY(sku_id) REFERENCES SKU(sku_id);
...
ALTER TABLE CYCLE_COUNT_DETAIL ADD CONSTRAINT "fk_cycle_count_detail_reason_id" FOREIGN KEY(discrepancy_reason_id) REFERENCES DISCREPANCY_REASON(reason_id);
...
CREATE INDEX "idx_inventory_balance_allocation" ON INVENTORY_BALANCE(warehouse_id, sku_id)
...
CREATE INDEX "idx_inventory_snapshot_warehouse_date" ON INVENTORY_SNAPSHOT(warehouse_id, snapshot_date);
