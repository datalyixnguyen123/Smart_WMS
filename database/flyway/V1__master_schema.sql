...
COMMENT ON COLUMN WAREHOUSE.gross_volume IS 'total max volume of warehouse(unit: m^3)';
...
COMMENT ON COLUMN SKU.is_fragile IS 'If set to true, the system alerts staff during picking/packing process to wrap the item in bubble wrap and apply warning tape';
COMMENT ON COLUMN SKU.is_hazardous IS 'Applies to chemical, flammable, or explosive goods. The WMS uses this flag to prevent the Sku from being stored in standard rack locations, requiring it instead to be placed in a designated segregated storage area';
COMMENT ON COLUMN SKU.packaging_level IS 'BASE_UNIT, INNER_PACK, CASE, PALLET, DISPLAY';
...
COMMENT ON COLUMN EQUIPMENT.equipment_type IS 'FORKLIFT, REACH_TRUCK, PALLET_JACK, RF_SCANNER etc';
...
COMMENT ON COLUMN STORAGE_POLICY.policy_name IS 'Displayed on the configuration interface (e.g., Prioritize fast-moving goods, Hazardous chemicals, Extra-heavy items on bottom shelves).';
...
CREATE TABLE SKU_ATTRIBUTE(sku_attribute_id uuid PRIMARY KEY, sku_id uuid NOT NULL, attribute_id uuid NOT NULL, attribute_option_id uuid, attribute_value varchar(255), created_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP, version int4 NOT NULL DEFAULT 0,
    CONSTRAINT ck_sku_attribute_value
        CHECK ((attribute_option_id IS NOT NULL AND attribute_value IS NULL) OR (attribute_option_id IS NULL AND attribute_value IS NOT NULL)));
